...
class Adp_crude extends CI_Model{
	...
	public function insertRow($table,$array){
		if(isset($array['name']) && !$this->is_unique($table,'name',$array['name'])){
			$this->session->set_flashdata('alert',['type'=>'danger','msg'=>$array['name'].' Category Name already exists']);
			return false;
		}elseif(isset($array['slug']) && !$this->is_unique($table,'slug',$array['slug'])){
			...
			$this->session->set_flashdata('alert',['type'=>'success','msg'=>$array['name'].' Category added successfully','last_action_id'=>$this->db->insert_id()]);
			return $response;
		}
